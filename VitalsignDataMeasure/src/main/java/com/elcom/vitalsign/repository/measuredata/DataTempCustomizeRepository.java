...
public class DataTempCustomizeRepository {
    ...
    public List<TempDataByHealthProfile> findDataTempLatestByHealthProfileIdLst(String[] healthProfileIdLst) {
        ...
        try {
            String sql = " SELECT user_profile_id AS healthProfileId, temp, ts, UNIX_TIMESTAMP(last_updated_at) AS lastUpdatedAt " +
                         " FROM data_temp_latest " +
            ...
            return session.createNativeQuery(sql)
                    ...
                    .addScalar("temp", FloatType.INSTANCE)
                    .addScalar("ts", LongType.INSTANCE)
                    .addScalar("lastUpdatedAt", LongType.INSTANCE)
                    .setResultTransformer(Transformers.aliasToBean(TempDataByHealthProfile.class))
                    .getResultList();
        } catch (Exception ex) {
    ...
    public List<TempDataByHealthProfile> findDataTempWithRangeByHealthProfileId(String healthProfileId, String filterType) {
        ...
        try {
            ...
            if( partition==null )
                return null;
            String sql = " SELECT user_profile_id AS healthProfileId, temp, ts, UNIX_TIMESTAMP(created_at) AS lastUpdatedAt " +
                         " FROM data_temp PARTITION ("+ partition +") " +
    ...
    public void progressDataLatest(DataTemp item, String gateIdLst) {
        ...
        try {
            ...
            try {
                Query query = session.createNativeQuery(" SELECT id FROM data_temp_latest WHERE gate_id = ? AND user_profile_id = ? ");
...
//                query.executeUpdate();
                ...
                session.save(new DataTempLatest(item.getGateId(), item.getUserProfileId(), item.getMeasureId(), item.getTs(),
                       item.getTemp()));
            }
            tx.commit();
        } catch (Exception ex) {
