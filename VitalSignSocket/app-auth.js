...
io.sockets.on('connection', function (socket) {
  ...
  /* AUTHORIZATION */
  socket.auth = false;
  socket.on('auth0', function(data) {
    jwt.verify(data.token, _cfg.AUTH_TRANS_KEY, function (err, decoded) {
      if (err) {
        logger.info('auth0.ex: ' +err.name + ' --> ' + err.message);
        //next(new Error("token not authorized"));
      }else {
        if( data.uuid !== decoded.content ) {
          logger.error('auth0.token ['+decoded.content+'] not authorized with uuid[' + data.uuid + ']');
          //next(new Error("token not authorized with uuid"));
        }else {
  ...
  setTimeout(function() {
    if ( !socket.auth)  {
      logger.info("UnAuthorize, disconnecting socket ", socket.id);
